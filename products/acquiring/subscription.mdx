...
<Steps>
  <Step title="调用 `POST /subscriptions` 创建订阅计划。">
  </Step>
  <Step title="通过 Tokenization API 绑定用户支付方式。">
  </Step>
  <Step title="Waffo 在每个计费周期自动发起扣款并推送 Webhook 通知。">
  </Step>
  <Step title="商户根据 Webhook 事件处理扣款成功/失败逻辑。">
  </Step>
</Steps>
